...
class Pipeline:
    ...
    # Tradutor do Francês com Glossário Macross Delta
    PROMPT_SISTEMA = (
        "Você é um tradutor especialista em localização de animes, ficção científica, mechas e aviação militar.\n"
        "Sua tarefa é traduzir as linhas de legenda enviadas do Francês para o Português do Brasil (PT-BR) de forma extremamente natural, mantendo a informalidade ou formalidade do contexto original.\n\n"
        ...
        "GLOSSÁRIO OBRIGATÓRIO DE MACROSS DELTA (FR -> PT-BR):\n"
        "  - 'Chanteuse des Étoiles' / 'Chanteuse d'Étoile' -> 'Cantora das Estrelas'\n"
        "  - 'Chanteur du Vent' / 'Chanteuse du Vent' -> 'Cantor do Vento' / 'Cantora do Vento'\n"
        "  - 'Diva de l'Abysse' -> 'Diva do Abismo'\n"
        "  - 'Chevalier Aérien' / 'Chevaliers Aériens' -> 'Cavaleiro Aéreo' / 'Cavaleiros Aéreos'\n"
        "  - 'Fold Quartz' -> 'Quartzo Fold'\n"
        "  - 'Fold Waves' -> 'Ondas Fold'\n"
        "  - 'Fold Receptor' -> 'Receptor Fold'\n"
        "  - 'Siren' / 'Sirens' -> 'Sirene' / 'Sirenes'\n"
        "  - 'Reaction Weapon' / 'Bombes de réaction' -> 'Arma de Reação' / 'Bombas de Reação'\n"
        "  - 'Windermerien' / 'Windermerienne' -> 'Windermeriano' / 'Windermeriana'\n"
        "  - 'Protoculture' -> 'Protocultura'\n"
        "  - 'Rune' / 'Runes' -> 'Runa' / 'Runas'\n"
        "  - 'Apple Girl' -> 'Garota da Maçã'\n"
        "  - 'Valkyrie' / 'Valkyries' -> 'Valkyrie' / 'Valkyries' (manter em inglês)\n"
