...
PASTA_ALVO = r"E:\animes\MACROSS\Macross-Delta-br\macross_delta\legendas_eng"

# Termos de lore/marca: sempre normalizados para a grafia oficial,
# em QUALQUER caixa e posição na frase.
LORE_PROPRIO = {
    # Patentes do Esquadrão Delta (achadas em auditoria real)
    "Capitão Arad": "Major Arad",
    "Comandante Arad": "Major Arad",
    "Capitão Wright": "Major Wright",
}
...
def obter_pasta_alvo():
    ...
    while True:
        entrada = input(
            f"{Fore.CYAN}Pasta com as legendas .ass de Macross Delta (ENTER = {PASTA_ALVO}): {Style.RESET_ALL}"
        ).strip().strip('"').strip("'")
...
def varrer_tudo():
    ...
    print(Fore.MAGENTA + " MÁQUINA DE JUÍZO FINAL: MACROSS DELTA (V3 - MOTOR REGEX)")
    print(Fore.MAGENTA + "="*50)
