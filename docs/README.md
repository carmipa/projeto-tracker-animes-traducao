...
| Documento | Conteúdo |
|:---|:---|
| [**🏗️ Arquitetura do pipeline**](arquitetura.md) | Mapa das fases 01–12 + diagramas de todas as esteiras (A–N) |
...
| 07 | [🧹/🩹 Higienização e reparo](modulo-fase-07.md) | `07_higienizacao_e_reparo_de_traducao/` |
...
|:---:|:---|:---|:---|
| **A** | 05a → [07] → 12 → [10] | Eighty-Six, ASS embutido (inglês) | [Arquitetura](arquitetura.md#esteira-a--eighty-six-ass-embutido-inglês) |
...
| **D** | 05b → [07] → 12 → [10] | Macross Delta TV, ASS embutido (francês) | [Arquitetura](arquitetura.md#esteira-d--macross-delta-tv-tradução-francês--pt-br) |
| **E** | 05b → [07] → 12 → 10 | Macross Delta Filme 2 (francês) | [Arquitetura](arquitetura.md#esteira-e--macross-delta-filme-2-francês) |
| **F** | 02 → 05a → [07] → 12 | Lote ASS pré-extraído (Gundam Reconguista) | [Arquitetura](arquitetura.md#esteira-f--lote-ass-pré-extraído-gundam-reconguista) |
| **G** | 02 → 05a → [06] → [07] → 12 → [10] | Gundam Unicorn (especializada) | [Arquitetura](arquitetura.md#esteira-g--gundam-unicorn-especializada) |
| **H** | 02 → 05a → [07] → 11 → 12 → [10] | Guilty Crown (correção de nomes e cores) | [Arquitetura](arquitetura.md#esteira-h--guilty-crown-correção-de-nomes-e-cores-de-músicas) |
...
`[10]`/`[06]`/`[07]` = passo opcional/condicional. **Fase 07** é transversal — normaliza lore/gramática (higienização) e repara erros de tradução das legendas por título. **Fases 01, 03, 08, 09** são auxiliares, usadas conforme necessário em qualquer esteira. **Fases 05a/05b/05c/05c-2** são variantes de motor de IA do mesmo papel (extrai/traduz), escolhidas pelo idioma de origem e pelo título. **Fases 06, 07, 11** são reparos e correções pós-tradução. A **Fase 10** é o catálogo de revisões finais por título.
...
**Episódios MKV (Esteira A, inglês, Gemma 4B):** Fases [01](modulo-fase-01.md) (opcional) → [05a](modulo-fase-05a.md) → [07](modulo-fase-07.md) → [12](modulo-fase-12.md)

**Episódios MKV (Esteira D, francês, Mistral Nemo):** Fases [01](modulo-fase-01.md) (opcional) → [05b](modulo-fase-05b.md) → [07](modulo-fase-07.md) → [12](modulo-fase-12.md)

**Filme/SRT (Esteira B):** [Pipeline SRT](pipeline-srt.md) → Fases 05a → 04 → 12

**Gundam The Origin (Esteira I, legenda chinesa):** [Fase 02](modulo-fase-02.md) → [Fase 05c](modulo-fase-05c.md) → [Fase 07](modulo-fase-07.md) → [Fase 10](modulo-fase-10.md) → [Fase 12](modulo-fase-12.md)

**Problemas:** [Logs](logs-e-auditoria.md) → [Solução de problemas](solucao-de-problemas.md)

**Falhas de tradução, lore ou formatação pós-tradução:** [Fase 07](modulo-fase-07.md) (higienização por título e reparo via IA) ou [Fase 11](modulo-fase-11.md) (correção offline, Guilty Crown)

**Erros de revisão final:** [Fase 10](modulo-fase-10.md) (catálogo de scripts de QA + remux)
