...
PASTA_ANIME_PADRAO = r"C:\animes\Gundam_Zeta"
...
def processar_legendas(
    ...
    _emit(f"\n{Fore.CYAN}=== REVISÃO EXTREMA ZETA GUNDAM ===")
        ...
        for idx_linha, linha in enumerate(linhas_ptbr, 1):
            ...
            # 3) Higienização geral (não karaoke)
                ...
                if texto_hig != texto_ptbr:
                    ...
                    modificada = True
                    if contem_lore:
                        motivo = "Correção de lore Zeta Gundam"
                        tipo = "LORE"
                    else:
...
def _salvar_log(log_modificacoes, stats, dry_run, pasta_ptbr=None, pasta_eng=None, pasta_out=None):
    ...
    caminho_json = os.path.join(pasta_logs, f"stats_gundam_zeta_{ts}.json")
    payload = {
        "timestamp": ts,
        "modo": modo,
        "pastas": {
    ...
    caminho_log = os.path.join(pasta_logs, f"revisao_gundam_zeta_{ts}.txt")
    try:
        with open(caminho_log, "w", encoding="utf-8") as fl:
            fl.write(f"RELATÓRIO REVISÃO EXTREMA ZETA GUNDAM ({modo})\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Revisão extrema Zeta Gundam — lore, karaoke, tags ASS",
        epilog=(
            "Exemplos:\n"
            "  python revisao_legenda_gundam_zeta.py\n"
            "  python revisao_legenda_gundam_zeta.py \"C:\\animes\\Gundam_Zeta\\legendas_ptbr\"\n"
            "  python revisao_legenda_gundam_zeta.py --entrada PT --eng ENG --saida SAIDA\n"
            "  python revisao_legenda_gundam_zeta.py --arquivo ep01_PTBR.ass --eng ENG --saida SAIDA\n"
        ),
    ...
    _emit("=" * 80)
    _emit(f"{Fore.CYAN}  REVISÃO EXTREMA — ZETA GUNDAM")
    _emit("=" * 80)
