...
ARQUIVO_INFO = None   # Definido dentro da pasta de saída (legendas_ptbr) em executar_pipeline_lote()
DEBUG_FILE = None     # Definido dentro da pasta de saída (legendas_ptbr) em executar_pipeline_lote()
_debug_salvo = False

logger = logging.getLogger("sidonia_translator")
...
def _configurar_logger(pasta_saida, timestamp_execucao):
    ...
    caminho_log = os.path.join(pasta_saida, f"log_traducao_sidonia_{timestamp_execucao}.log")
...
SYSTEM_PROMPT = (
    ...
    "REGRAS CRÍTICAS:\n"
    "1. Mantenha a coerência terminológica de Knights of Sidonia (Sidonia no Kishi). Ex: 'Gauna' (alienígenas), 'Garde' (robôs/mechas), 'Kabizashi' (arma/lança), 'Heigus' ou 'Higgs' -> 'Partícula Heigus', 'Toha Heavy Industries' -> 'Indústrias Pesadas Toha', 'Seed Ship' -> 'Nave Semente'. Nomes de personagens devem ser mantidos sem tradução.\n"
...
def traduzir_bloco_ia(bloco, nome_arquivo="?"):
    ...
    while tentativa < max_tentativas:
        try:
            ...
            if resposta.status_code != 200:
                print(f"\n{Fore.RED}[ERRO DETALHADO] LM Studio respondeu HTTP {resposta.status_code}: {resposta.text}")
                ...
                if i < len(traducoes) and traducoes[i] and traducoes[i].lower() != texto_orig.lower():
                    ...
                    for idx_tag, tag in enumerate(tags):
                        ...
                        if marcador in trad:
                            ...
                            # Escapa a tag para evitar 'bad escape \i' no re.sub
                            tag_escapada = tag.replace('\\', '\\\\')
                            trad = re.sub(rf'\[?[Tt]{idx_tag}\]?', tag_escapada, trad, count=1)
                    resultados.append((idx, f"{meta}{trad}\n", False))
                else:
                    # Se falhou ou retornou o mesmo texto (ex: Nomes), usa o texto original sem sujar a tela
                    resultados.append((idx, f"{meta}{texto_orig}\n", True))
            return resultados
            ...
            if tentativas_offline < max_tentativas_offline:
                ...
                logger.error(f"[{nome_arquivo}] Bloco {indices_u[0]}-{indices_u[-1]} | LM Studio permaneceu OFFLINE após {max_tentativas_offline} tentativas ({sum(i*20 for i in range(1, max_tentativas_offline))}s de espera total) | {e}")
                ...
                for idx, meta, texto, _ in zip(indices_u, metadados_u, textos_u, tags_u):
                    resultados.append((idx, f"{meta}{texto}\n", True))
                return resultados
...
def executar_pipeline_lote():
    print("=" * 80)
    print(f"{Fore.CYAN}    ESTEIRA BATCH {BATCH_SIZE}L/CHAMADA | THREADS={MAX_THREADS} | ASS -> ASS PTBR (SIDONIA EDITION)")
    print("=" * 80)

    verificar_lm_studio()

    caminho_padrao_origem = r"C:\TRACKER-ANIMES\animes\Sidonia no Kishi (Knights of Sidonia)\Movie\legendas_eng"
    pasta_origem = obter_diretorio_operador("Pasta com legendas ENG (.ass)", caminho_padrao_origem)

    caminho_padrao_saida = r"C:\TRACKER-ANIMES\animes\Sidonia no Kishi (Knights of Sidonia)\Movie\legendas_ptbr"
    ...
    ARQUIVO_INFO = os.path.join(pasta_saida, "info_traducao_sidonia_ass.txt")
    DEBUG_FILE = os.path.join(pasta_saida, "debug_last_failure_sidonia_ass.txt")
            ...
            for i, linha in enumerate(linhas_originais):
                if linha.startswith("Dialogue:"):
                    ...
                    if len(partes) == 10:
                        ...
                        # Filtros Avançados: Ignorar linhas que não precisam de tradução
                        texto_sem_tags = re.sub(r'\[T\d+\]', '', texto_limpo).strip()
                        is_drawing = bool(re.match(r'^m\s+-?\d+', texto_sem_tags))
                        has_letters = bool(re.search(r'[a-zA-Z]', texto_sem_tags))
                        
                        if is_drawing or not has_letters:
                            mapa_linhas_finais[i] = linha
                            continue
