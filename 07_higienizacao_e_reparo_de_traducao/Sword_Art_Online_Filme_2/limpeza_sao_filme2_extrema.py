...
PASTA_ALVO = r"C:\animes\SAO"

# Termos de lore/marca: sempre normalizados para a grafia oficial,
# em QUALQUER caixa e posição na frase.
LORE_PROPRIO = {
    # Termos e nomes fundamentais de Sword Art Online
    "Aincrad": "Aincrad",
    "NerveGear": "NerveGear",
    "Link Start": "Link Start",
    "Laughing Coffin": "Caixão Sorridente",
    "Aincrad Liberation Squad": "ALS",
    "Dragon Knights Brigade": "DKB",
    
    # Correção do clássico e bizarro erro do "Rogério"
    "Rogério": "Copiado",
    "rogério": "copiado",
    
    # Correções de MMORPG / MMORPG Party (evita tradução literal "Festa" para grupo de jogo)
    "a minha festa": "o meu grupo",
    "minha festa": "meu grupo",
    "sua festa": "seu grupo",
    "esta festa": "este grupo",
    "essa festa": "esse grupo",
    "uma festa": "um grupo",
    "a festa": "o grupo",
    "festa de incursão": "grupo de incursão",
    "festa de invasão": "grupo de incursão",
    "festa do chefe": "grupo do chefe",
    "festa do boss": "grupo do chefe",
    "convite de festa": "convite de grupo",
    "entrar na festa": "entrar no grupo",
    "juntar-se à festa": "entrar no grupo",
    "criar uma festa": "criar um grupo",
    "fazer uma festa": "criar um grupo",
    "membros da festa": "membros do grupo",
...
def obter_pasta_alvo():
    ...
    while True:
        entrada = input(
            f"{Fore.CYAN}Pasta com as legendas .ass/.srt de SAO Filme 2 (ENTER = {PASTA_ALVO}): {Style.RESET_ALL}"
        ).strip().strip('"').strip("'")
...
def varrer_tudo():
    ...
    print(Fore.MAGENTA + " MÁQUINA DE JUÍZO FINAL: SWORD ART ONLINE FILME 2 (V3 - MOTOR REGEX)")
    print(Fore.MAGENTA + "="*50)
    ...
    padrao_timecode_srt = re.compile(r'-->')
    padrao_indice_srt = re.compile(r'^\d+$')
    ...
    for arq in alvos:
        ...
        for i, linha in enumerate(linhas):
            if eh_ass:
                ...
                # SRT parsing simples e seguro
                texto = linha.rstrip('\n').rstrip('\r')
                if not texto.strip() or padrao_indice_srt.match(texto.strip()) or padrao_timecode_srt.search(texto):
                    continue
                texto_limpo, modificado = higienizar_linha(texto)
                if modificado:
