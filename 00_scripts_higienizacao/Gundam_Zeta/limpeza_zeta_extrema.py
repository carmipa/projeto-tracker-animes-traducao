...
GRAMATICA_E_GAFES = {
    ...
    # As 3 combinações erradas possíveis de pronome+conjugação para cada verbo.
    # ----------------------------------------------------
    ...
    # (clássica pegadinha de LLM traduzindo do inglês, que não tem gênero gramatical)
    # ----------------------------------------------------
...
def _balancear_tag(t, abre, fecha):
    # Conta aberturas x fechamentos (não só presença) - se a IA abriu 2x e fechou 1x,
    # a verificação antiga ("fechamento existe na frase?") deixava passar o erro.
...
def higienizar_linha(texto):
    ...
    # (genérico: cobre {\an8}{\an8}, {\i1}{\i1}, {\pos(..)}{\pos(..)}, etc., sem precisar listar uma por uma)
    t = re.sub(r'(\{\\[^{}]+\})\1+', r'\1', t)
...
def varrer_tudo():
    ...
    print(Fore.MAGENTA + " INICIANDO A MÁQUINA DE JUÍZO FINAL: ZETA GUNDAM (V3 - MOTOR REGEX)")
    print(Fore.MAGENTA + "="*50)
    ...
    print(Fore.MAGENTA + "="*50)
    print(f"{Fore.MAGENTA} TOTAL DE ANOMALIAS DESTRUÍDAS EM ZETA: {total_correcoes}")
    print(Fore.MAGENTA + "="*50)
