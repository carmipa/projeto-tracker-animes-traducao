...
class Pipeline:
    ...
    # Tradutor do Francês (FR -> PT-BR)
    PROMPT_SISTEMA = (
        "Você é um tradutor especialista em animes de ficção científica, mechas e aviação militar.\n"
        "Traduza APENAS o texto fornecido do francês para português do Brasil (PT-BR).\n"
        "Regras estritas:\n"
        ...
        "3. Preserve termos técnicos militares de forma natural e fluida para a franquia Macross."
    )
    ...
    def _traduzir_lote(self, linhas: list, tentativa=1) -> dict:
        ...
        if not traduzidas:
            raise RuntimeError("Impossível extrair traduções")
    ...
    def processar_legenda(self, ass_path: str, saida_path: str) -> bool:
        ...
        try:
            ...
            if not textos:
                self.log.aviso("Nenhum diálogo encontrado")
                return False
    ...
    try:
        ...
        for idx, nome_mkv in enumerate(mkv_files, 1):
            ...
            if pipe.processar_legenda(ass_temp, ass_final):
                ...
                # Mensagem detalhada com temporizador por arquivo e tempo total acumulado
