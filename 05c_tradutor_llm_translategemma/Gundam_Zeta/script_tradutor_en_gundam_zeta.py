...
ARQUIVO_CACHE = os.path.join(BASE_DIR, "traducao_cache_gundam_zeta_en.json")
ARQUIVO_STATS = os.path.join(LOGS_DIR, f"stats_en_gundam_zeta_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json")
...
3. DO NOT translate proper nouns or terms from the Universal Century. KEEP THEM IN ENGLISH:
   - Factions/Organizations: AEUG, Titans, Karaba, Earth Federation, Axis Zeon, Anaheim Electronics.
   - Ships: Argama, Radish, Alexandria, Jupitris.
   - Characters: Kamille Bidan, Quattro Bajeena, Char Aznable, Amuro Ray, Jerid Messa, Paptimus Scirocco, Haman Karn, Emma Sheen, Reccoa Londe, Bask Om, Jamitov Hymen, Four Murasame, Rosamia Badam, Fa Yuiry, Yazan Gable, Bright Noa, Henken Bekkener.
   - Lore/Mechas: Zeta Gundam, Gundam Mk-II, Hyaku Shiki, The O, Qubeley, Psycho Gundam, Methuss, Gaplant, Hambrabi, Mobile Suit, Newtype, Minovsky Particle, Cyber-Newtype.
4. Output NOTHING but the final translated text. No pleasantries, no notes.
5. You MUST return each translated line with its exact original index prefix (e.g., [0] <translated text>)."""
    ...
    try:
        ...
        for idx_local, trecho_orig_masc in enumerate(trechos_para_traduzir):
            ...
            if not texto_pt_masc:
                resultados[idx_global] = "[ERRO_TRADUCAO] " + trechos[idx_global]
                estatisticas["erros"] += 1
                continue
